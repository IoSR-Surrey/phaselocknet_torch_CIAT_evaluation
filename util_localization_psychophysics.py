'''
This script, containing utility functions was copied from
the original phaselocknet repository:
https://github.com/msaddler/phaselocknet/blob/main/util_localization_psychophysics.py
'''
...
def normalize_angle(angle, lower=-180, upper=180):
    ...
    normalized_angle[IDX_ABOVE] = lower + np.mod(
        np.abs(angle[IDX_ABOVE] + upper), np.abs(lower) + np.abs(upper)
    )
    IDX_BELOW = np.logical_or(angle < lower, angle == upper)
    normalized_angle[IDX_BELOW] = upper - np.mod(
        np.abs(angle[IDX_BELOW] - lower), np.abs(lower) + np.abs(upper)
    )
...
def fit_normcdf(xvals, yvals, mu=0.0, sigma=None):
    ...
    sigma_opt, sigma_opt_cov = scipy.optimize.curve_fit(
        normcdf, xvals, yvals, sigma=sigma
    )
...
def get_azim_to_itd_mapping(fn="data/misc/kemar_hrtf_itds.pkl"):
    ...
    azim_to_itd = lambda azim: np.interp(
        ((azim + 180) % 360) - 180, xp=list_azim, fp=list_itd
    )
    return azim_to_itd
...
def get_azim_to_ild_mapping(fn="data/misc/kemar_hrtf_ilds.pkl", freq="low"):
    ...
    azim_to_ild = lambda azim: np.interp(
        ((azim + 180) % 360) - 180, xp=list_azim, fp=list_ild
    )
    return azim_to_ild
...
def experiment_simple(
    ...
    if prior is not None:
        ...
        if verbose:
            print(
                "Prior modified {} of {} predicted labels".format(
                    changed, label.shape[0]
                )
            )
...
def experiment_itd_ild_weighting(
    ...
    df = pd.DataFrame(
        {
            ...
            "azim": azim,
            "ild_bias_imposed": -1
            * eval_dict["ild"],  # Sign change because +ILD = louder right ear
            "itd_bias_imposed": 1e6
            * eval_dict["itd"]
            / sr,  # Convert `ITD` from samples to microseconds
            "cutoff_low": eval_dict["noise_low"],
            "cutoff_high": eval_dict["noise_high"],
            "noise_idx": eval_dict["idx"],
            "band": list(
                zip(list(eval_dict["noise_low"]), list(eval_dict["noise_high"]))
            ),
        }
    )
    ...
    IDX_NONPHYSIOLOGICAL = np.logical_or.reduce(
        [
            np.logical_and(
                np.abs(eval_dict["ild"]) >= 10, np.abs(azim_true) > 40
            ),  # From francl code
            np.logical_and(
                np.abs(eval_dict["ild"]) >= 20, np.abs(azim_true) > 20
            ),  # From francl code
            np.logical_and(
                np.abs(eval_dict["itd"]) >= 18, np.abs(azim_true) > 40
            ),  # From francl code
            np.logical_and(
                np.abs(eval_dict["itd"]) >= 22, np.abs(azim_true) > 20
            ),  # From francl code
        ]
    )
    ...
    df_unbiased = df[
        np.logical_and(df["ild_bias_imposed"] == 0, df["itd_bias_imposed"] == 0)
    ]
    df_unbiased = (
        df_unbiased.groupby(["azim_true", "band"])["mean"].mean().reset_index()
    )
    ...
    df_itd = df[
        df["ild_bias_imposed"] == 0
    ].copy()  # Consider only rows with bias ILD = 0
    if exclude_by_std:
        ...
        if verbose:
            print(
                f"Excluding {IDX_exclude.sum()} of {len(IDX_exclude)} conditions from ITD expt due to std > {threshold}"
            )
        df_itd = df_itd[~IDX_exclude]
    df_itd["itd_bias_response"] = df_itd.apply(itd_residual_fcn, axis=1)
    df_itd = (
        df_itd.groupby(
            ["cutoff_low", "cutoff_high", "band", "fn_eval", "itd_bias_imposed"]
        )
        .agg({"itd_bias_response": "mean"})
        .reset_index()
    )
    # ILD experiment (aggregate down to unique ild_bias_imposed values)
    df_ild = df[
        df["itd_bias_imposed"] == 0
    ].copy()  # Consider only rows with bias ITD = 0
    if exclude_by_std:
        if isinstance(exclude_by_std, bool):
            threshold = 14
        else:
        ...
        if verbose:
            print(
                f"Excluding {IDX_exclude.sum()} of {len(IDX_exclude)} conditions from ILD expt due to std > {threshold}"
            )
        df_ild = df_ild[~IDX_exclude]
    df_ild["ild_bias_response"] = df_ild.apply(ild_residual_fcn, axis=1)
    df_ild = (
        df_ild.groupby(
            ["cutoff_low", "cutoff_high", "band", "fn_eval", "ild_bias_imposed"]
        )
        .agg({"ild_bias_response": "mean"})
        .reset_index()
    )
...
def experiment_minimum_audible_angle(
    ...
    if os.path.exists(fn_results) and (not overwrite):
        ...
        eval_dict["azim_pred"], _ = func_label_to_azim_elev(
            probs_to_label(pred_prob, prior=prior)
        )
        ...
        df["maa"] = df.apply(
            lambda _: float(fit_threshold(_.azim_true, _.azim_pred)), axis=1
        )
        df["fn_eval"] = fn_eval
...
def experiment_itd_threshold(
    ...
    if os.path.exists(fn_results) and (not overwrite):
        ...
        df["itd_threshold"] = df.apply(
            lambda _: float(fit_threshold(_.itd, _.azim_pred)), axis=1
        )
        df["fn_eval"] = fn_eval
        df.to_pickle(fn_results)
        print(f"[WROTE]: {fn_results}")
...
def func_to_parallelize_minimum_audible_angle(
    ...
    """ """
    fn_eval = os.path.join(
        dir_model,
        f"EVAL{tag_ckpt}_PSYCHOPHYSICSv00_minimum_audible_angle_interpolated.json",
    )
...
def func_to_parallelize_itd_threshold(
    ...
    """ """
    fn_eval = os.path.join(
        dir_model, f"EVAL{tag_ckpt}_PSYCHOPHYSICSv00_itd_threshold.json"
    )
...
def func_to_parallelize_itd_ild_weighting(
    ...
    """ """
    fn_eval = os.path.join(
        dir_model, f"EVAL{tag_ckpt}_PSYCHOPHYSICSv00_itd_ild_weighting.json"
    )
...
def func_to_parallelize_spectral_smoothing(
    ...
    """ """
    fn_eval = os.path.join(
        dir_model, f"EVAL{tag_ckpt}_PSYCHOPHYSICSv00_spectral_smoothing.json"
    )
    ...
    df = (
        df.groupby(["fn_eval", "smoothed", "azim_true", "elev_true"])
        .agg({"azim_pred": "mean", "elev_pred": "mean"})
        .reset_index()
    )
...
def func_to_parallelize_precedence_effect_localization(
    ...
    fn_eval = os.path.join(
        dir_model,
        f"EVAL{tag_ckpt}_PSYCHOPHYSICSv00_precedence_effect_localization.json",
    )
    eval_dict = load_eval_dict(fn_eval)
    pred_prob = eval_dict.pop(key_pred_prob)
    azim, elev = func_label_to_azim_elev(np.arange(0, pred_prob.shape[1]))
...
def func_to_parallelize_new_ears(
    ...
    df["azim_true"] = fold_front_back(
        np.around(1 / 5 * df["foreground_azimuth"], decimals=0) * 5
    )
...
def func_to_parallelize_bandwidth_dependency(
    ...
    """ """
    fn_eval = os.path.join(
        dir_model, f"EVAL{tag_ckpt}_PSYCHOPHYSICSv00_bandwidth_dependency.json"
    )
...
def func_to_parallelize_mp_spectral_cues(
    ...
    """ """
    fn_eval = os.path.join(
        dir_model, f"EVAL{tag_ckpt}_PSYCHOPHYSICSv00_mp_spectral_cues.json"
    )
    azim, elev = func_label_to_azim_elev(np.arange(0, n_loc_classes))
    azim = normalize_angle(azim)
    prior = np.logical_and.reduce(
    ...
    df = (
        df.groupby(["fn_eval", "noise_low", "noise_high"])
        .agg({"percent_correct": "mean"})
        .reset_index()
    )
...
def func_to_parallelize_snr_dependency(
    ...
    """ """
    fn_eval = os.path.join(
        dir_model, f"EVAL{tag_ckpt}_v01_eval_mit_bldg46room1004_tenoise.json"
    )
    azim, elev = func_label_to_azim_elev(np.arange(0, n_loc_classes))
    azim = normalize_angle(azim)
    prior = np.logical_and.reduce(
...
def func_to_parallelize_speech_in_noise_in_reverb(
    ...
    """ """
    fn_eval = os.path.join(
        dir_model, f"EVAL{tag_ckpt}_PSYCHOPHYSICSv00_speech_in_noise_in_reverb_v04.json"
    )
    azim, elev = func_label_to_azim_elev(np.arange(0, n_loc_classes))
    azim = normalize_angle(azim)
    prior = np.logical_and.reduce(
    ...
    **kwargs,
    ...
    if (list_expt is None) or (tag_experiment in list_expt):
        df_human = pd.read_csv(
            os.path.join(dir_human_data, "mills_1958_minimum_audible_angle.csv")
        )
        df_human = df_human[df_human["f"] < 1e3]
        df_human["log_maa"] = np.log(df_human["maa"])
        df_human = util.flatten_columns(
            df_human.rename(
                columns={
                    "f": "f_ref",
                    "azim": "azim_ref",
        ...
        for regex_dir_model in list_regex_dir_model:
            ...
            with multiprocessing.Pool(min(workers, len(list_dir_model))) as p:
                func = functools.partial(
                    func_to_parallelize_minimum_audible_angle, **kwargs
                )
            ...
            print(
                f"Completed experiment `{tag_experiment}` for {len(list_dir_model)} `{regex_dir_model}` models"
            )
    ...
    if (list_expt is None) or (tag_experiment in list_expt):
        df_human = pd.read_csv(
            os.path.join(
                dir_human_data, "macpherson_middlebrooks_2002_itd_ild_weighting.csv"
            )
        )
        df_human = df_human.rename(columns={"subject": "fn_eval"})
        list_df_human = []
        ...
        for regex_dir_model in list_regex_dir_model:
            ...
            with multiprocessing.Pool(min(workers, len(list_dir_model))) as p:
                func = functools.partial(
                    func_to_parallelize_itd_ild_weighting, **kwargs
                )
                list_df = p.map(func, list_dir_model)
            df = pd.concat(list_df)
            df["tag_model"] = regex_dir_model
        ...
        df_results = pd.concat(df_results).reset_index(drop=True)
        df_results = (
            df_results.groupby(["tag_model", "band", "fn_eval"])
            .agg(
                {
        ...
        df_results["itd_bias_response"] = (
            df_results["itd_bias_imposed"] * df_results["itd_bias_weight"]
        )
        df_results["ild_bias_response"] = (
            df_results["ild_bias_imposed"] * df_results["ild_bias_weight"]
        )
        df_results = util.flatten_columns(
            df_results.groupby(["tag_model", "band"])
            .agg(
                {
    ...
    if (list_expt is None) or (tag_experiment in list_expt):
        ...
        df_human = pd.read_csv(
            os.path.join(dir_human_data, "kulkarni_colburn_1998_spectral_smoothing.csv")
        )
        df_human = util.flatten_columns(
            df_human.rename(columns={"subject": "fn_eval"})
            .groupby(["smoothed", "fn_eval"])
            .agg({"pct_correct": "mean"})
            .reset_index()
            .groupby("smoothed")
            .agg(
                {
        ...
        for regex_dir_model in list_regex_dir_model:
            ...
            with multiprocessing.Pool(min(workers, len(list_dir_model))) as p:
                func = functools.partial(
                    func_to_parallelize_spectral_smoothing, **kwargs
                )
                list_df = p.map(func, list_dir_model)
            df = pd.concat(list_df)
            df = (
                df.groupby(["fn_eval", "smoothed"])
                .agg(
                    {
                        key_metric: "mean"
            ...
            list_fn_eval = (
                df.groupby(["smoothed"]).agg({"fn_eval": list})["fn_eval"].values
            )
            df = (
                df.groupby(["smoothed"])
                .agg(
                    {
                        key_metric: ["mean", "sem", list]
                        for key_metric in list_key_metric
                        if key_metric in df.columns
                    }
                )
    ...
    if (list_expt is None) or (tag_experiment in list_expt):
        df_human = pd.read_csv(
            os.path.join(dir_human_data, "litovsky_godar_2010_precedence_effect.csv")
        )
        df_lead = (
            df_human[df_human.condition == 0]
            .rename(
                columns={
        ...
        for regex_dir_model in list_regex_dir_model:
            ...
            with multiprocessing.Pool(min(workers, len(list_dir_model))) as p:
                func = functools.partial(
                    func_to_parallelize_precedence_effect_localization, **kwargs
                )
                list_df = p.map(func, list_dir_model)
            df = pd.concat(list_df)
            df = df[
        ...
        df_results = df_results[df_results["delay"].isin(df_human.delay)].reset_index(
            drop=True
        )
        EXPERIMENT_DATAFRAMES[tag_experiment] = df_results.sort_index(axis=1)
    ...
    if (list_expt is None) or (tag_experiment in list_expt):
        df_human = pd.read_csv(
            os.path.join(dir_human_data, "hofman_etal_1998_new_ears.csv")
        )
        df_human = df_human.rename(columns={"subject": "fn_eval"})
        ...
        df_results = (
            df_results.groupby(
                ["tag_model", "fn_eval", "new_ears", "ears", "azim_true", "elev_true"]
            )
            .agg(
                {
                    "azim_pred": "mean",
        ...
        df_results = (
            df_results.groupby(
                ["tag_model", "fn_eval", "new_ears", "azim_true", "elev_true"]
            )
            .agg(
                {
                    "azim_pred_arr": list,
            ...
            .reset_index()
            .groupby(["tag_model", "new_ears", "azim_true", "elev_true"])
            .agg(
                {
                    "azim_pred_arr": list,
                    "elev_pred_arr": list,
        ...
        df_results = df_results.rename(
            columns={
                "azim_pred_arr_list": "azim_pred_arr",
                "elev_pred_arr_list": "elev_pred_arr",
            }
        )
        EXPERIMENT_DATAFRAMES[tag_experiment] = df_results.sort_index(axis=1)
    ...
    if (list_expt is None) or (tag_experiment in list_expt):
        df_human = pd.read_csv(
            os.path.join(dir_human_data, "yost_zhong_2014_bandwidth_dependency.csv")
        )
        ...
        for regex_dir_model in list_regex_dir_model:
            ...
            with multiprocessing.Pool(min(workers, len(list_dir_model))) as p:
                func = functools.partial(
                    func_to_parallelize_bandwidth_dependency, **kwargs
                )
                list_df = p.map(func, list_dir_model)
            df = pd.concat(list_df)
            df = df[df.f_ref.isin(list_f_ref)]
            df = (
                df.groupby(["fn_eval", "bandwidth"])
                .agg(
                    {
                ...
                .reset_index()
                .rename(columns={"deg_azim_err": "rms_error"})
            )
            df = util.flatten_columns(
    ...
    if (list_expt is None) or (tag_experiment in list_expt):
        df_human = pd.read_csv(
            os.path.join(
                dir_human_data, "hebrank_wright_1974_median_plane_spectral_cues.csv"
            )
        )
        df_human["condition"] = df_human["condition"].map(
        ...
        df_results = pd.concat(df_results).reset_index(drop=True)
        df_results = util.flatten_columns(
            df_results.groupby(["tag_model", "fn_eval", "condition", "cutoff"])
            .agg({"percent_correct": "mean"})
            .reset_index()
            .groupby(["tag_model", "condition", "cutoff"])
            .agg({"percent_correct": ["mean", "sem", list]})
            .reset_index(),
            sep="_",
        )
        EXPERIMENT_DATAFRAMES[tag_experiment] = df_results.sort_index(axis=1)
    ...
    if (list_expt is None) or (tag_experiment in list_expt):
        ...
        df_results = (
            df_results.groupby(["tag_model", "fn_eval", "snr"])
            .agg({key_metric: "mean" for key_metric in list_key_metric})
            .reset_index()
        )
        df_results = util.flatten_columns(
            df_results.groupby(["tag_model", "snr"])
            .agg({key_metric: ["mean", "sem", list] for key_metric in list_key_metric})
            .reset_index(),
            sep="_",
        )
    ...
    if (list_expt is None) or (tag_experiment in list_expt):
        ...
        for regex_dir_model in list_regex_dir_model:
            ...
            with multiprocessing.Pool(min(workers, len(list_dir_model))) as p:
                func = functools.partial(
                    func_to_parallelize_speech_in_noise_in_reverb, **kwargs
                )
                list_df = p.map(func, list_dir_model)
            df = pd.concat(list_df)
            df["tag_model"] = regex_dir_model
        ...
        df_results = (
            df_results.groupby(["tag_model", "fn_eval", "index_room", "snr"])
            .agg({key_metric: "mean" for key_metric in list_key_metric})
            .reset_index()
        )
        ...
        df_results = util.flatten_columns(
            df_results.groupby(["tag_model", "index_room", "snr"])
            .agg(dict_agg)
            .reset_index(),
            sep="_",
        )
...
def func_to_parallelize_bootstrap(
    bootstrap_IDX, y_human=None, list_y_model=None, metric_function=None
):
    """ """
...
def compare_localization_experiment(
    ...
    for tag_model in sorted(set(df["tag_model"].unique()).difference({"human"})):
        if tag_expt == "maa_azimuth":
            ...
            df = (
                df.groupby(["tag_model", "new_ears"], group_keys=False)
                .apply(grid_position)
        ...
        for key_y_human, key_y_model in zip(list_key_y_human, list_key_y_model):
            ...
            if interp:
                ...
                IDX = np.logical_and.reduce(
                    [~np.isnan(x), ~np.isnan(yh), ~np.isnan(ym)]
                )
                x = x[IDX]
                yh = yh[IDX]
                ym = ym[IDX]
            if interp:
                list_ym = np.array(
                    list(df_comparison[f"{key_y_model}_list_model"].values[IDX])
                ).T
            else:
                list_ym = np.array(
                    list(df_comparison[f"{key_y_model}_list_model"].values)
                ).T
            y_human.append(yh)
            y_model.append(ym)
        ...
        for metric_key, metric_function in dict_metric_function.items():
            ...
            if workers is None:
                ...
                individual_list_metric_val = [
                    func([_]) for _ in range(len(list_y_model))
                ]
            else:
                with multiprocessing.Pool(workers) as pool:
                    bootstrap_list_metric_val = pool.map(func, list_bootstrap_IDX)
                    individual_list_metric_val = pool.map(
                        func, [[_] for _ in range(len(list_y_model))]
                    )
        ...
        for metric_key in dict_metric_function.keys():
            ...
            to_append[f"bootstrap_list_{metric_key}"] = bootstrap_list_metric[
                metric_key
            ]
